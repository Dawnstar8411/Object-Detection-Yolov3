...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    # Sort by objectness
    i = np.argsort(-conf)  # from min to max
    ...
    for c in tqdm.tqdm(unique_classes, desc="Computing AP"):
        ...
        if n_p == 0 and n_gt == 0:
            ...
            r.append(recall_curve[-1])  # recall of class c

            # Precision
            precision_curve = tpc / (tpc + fpc)
            p.append(precision_curve[-1])  # precision of class c
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    nB = pred_boxes.size(0)   # numb_samples
    nA = pred_boxes.size(1)   # num_anchors
    nC = pred_cls.size(-1)    # num_classes
    nG = pred_boxes.size(2)   # num_height
    ...
    target_boxes = target[:, 2:6] * nG  # (0,1) to real scale
    gxy = target_boxes[:, :2]
    gwh = target_boxes[:, 2:]
